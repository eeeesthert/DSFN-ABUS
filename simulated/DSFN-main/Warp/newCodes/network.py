...
#     depth_mesh_motion = torch.stack([mesh_motion[...,0]*img_w/512, mesh_motion[...,1]*img_h/512], 3)
    ...
    # depthInput*_tensor shape [B,1,H,W].value [0,1]
    depthInput1_tensor = depthInput1_tensor.float()
    depthInput2_tensor = depthInput2_tensor.float()

    # view2 H warp -->view1(output_H)
    depth_warp_H = torch_homo_transform.transformer(depthInput2_tensor, H_mat, (img_h, img_w))
    # view2 TPS warp(warp_mesh)
    depth_warp_TPS = torch_tps_transform.transformer(depthInput2_tensor, norm_mesh, norm_rigid_mesh, (img_h, img_w))
    ...
    out_dict.update(
        ...
        mesh2=mesh,
        overlap=overlap,
        depth_warp1=depth_warp_H,    # warp_H  mask
        depth_warp2=depth_warp_TPS   # warp_mesh  mask
    )
